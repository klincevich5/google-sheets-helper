...
def process_full_rotation(values, source_page_area=None):
    """
    Ищет строку, содержащую 'shift' в колонке A (index 0),
    и возвращает значения до неё включительно.
    """
    try:
        for i, row in enumerate(values):
            if len(row) > 0 and "shift" in str(row[0]).strip().lower():
                return values[:i + 1]
...
def process_full_turkish_rotation(values, source_page_area=None):
    """
    Ищет блок между первой и второй строкой, содержащей "shift" в колонке A (index 0),
    и добавляет заголовок и Replacements TURKISH с обеих сторон.
    """
    try:
        start_index = None
