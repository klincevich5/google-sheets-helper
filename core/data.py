...
def load_rotationsinfo_tasks(session, log_file):
    ...
    try:
        ...
        log_error(log_file, "define_tasks", None, "fail", f"Ошибка при загрузке шаблонов: {e}")
        raise
    ...
    try:
        ...
        log_error(log_file, "define_tasks", None, "fail", f"Ошибка при загрузке задач из БД: {e}")
        raise
    ...
    if new_tasks:
        try:
            ...
            log_error(log_file, "define_tasks", None, "fail", f"Ошибка при сохранении новых задач: {e}")
            raise
    ...
    try:
        ...
        log_error(log_file, "define_tasks", None, "fail", f"Ошибка при загрузке активных задач: {e}")
        raise
...
def load_sheetsinfo_tasks(session, log_file):
    ...
    try:
        ...
        log_error(log_file, "define_tasks", None, "fail", f"Ошибка при загрузке шаблонов: {e}", exc=e)
        raise
    ...
    try:
        ...
        log_error(log_file, "define_tasks", None, "fail", f"Ошибка при загрузке задач из БД: {e}", exc=e)
        raise
    ...
    if new_tasks:
        try:
            ...
            log_error(log_file, "define_tasks", None, "fail", f"Ошибка при сохранении новых задач: {e}", exc=e)
            raise
    ...
    try:
        ...
        log_error(log_file, "define_tasks", None, "fail", f"Ошибка при загрузке активных задач: {e}", exc=e)
        raise
