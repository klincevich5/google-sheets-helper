...
from config import SHEETS_LOG_FILE, SCAN_INTERVAL_SECONDS, WARSAW_TZ
from datetime import datetime

def scan_process(process):
    log_to_file(SHEETS_LOG_FILE, f"🔍 Скан: ID={process['id']} — {process['name_of_process']}")
    time.sleep(1)  # заглушка
    update_last_scan("SheetsInfo", process["id"])
    log_to_file(SHEETS_LOG_FILE, f"✅ Завершено: ID={process['id']}")
...
def SheetsInfo_scanner():
    ...
    while True:
        log_to_file(SHEETS_LOG_FILE, "🔄 Проверка задач...")
        ...
        for task in tasks:
            ...
            if not doc_id:
                log_to_file(SHEETS_LOG_FILE, f"⚠️ Пропуск задачи без актуального doc_id: ID={task['id']}")
                continue

            log_to_file(SHEETS_LOG_FILE, f"📦 ID={task['id']} | 🆔 {doc_id} | 📑 {task['source_page_name']} | ⚙ {task['get_data_method']}")
            if doc_id:
        ...
        for doc_id, task_list in doc_to_tasks.items():
            log_to_file(SHEETS_LOG_FILE, f"🔄 Обработка ID={doc_id} с {len(task_list)} задачами"    )
            batch_ranges = build_batchget_ranges(task_list, SHEETS_LOG_FILE).get(doc_id, set())
            if not batch_ranges:
                log_to_file(SHEETS_LOG_FILE, f"⚠️ Пропуск ID={doc_id} — нет ranges")
                continue

            log_to_file(SHEETS_LOG_FILE, f"🔄 Выполняется batchGet для ID={doc_id}, диапазонов: {len(batch_ranges)}")
            values_map = fetch_batch_values(doc_id, list(batch_ranges))
            log_to_file(SHEETS_LOG_FILE, f"📊 Получено {len(values_map)} страниц от ID={doc_id}")

            for row in task_list:
                log_to_file(SHEETS_LOG_FILE, f"📦 ID={row['id']} | 🆔 {doc_id} | 📑 {row['source_page_name']} | ⚙ {row['get_data_method']}")
                ...
                if actual_range:
                    log_to_file(SHEETS_LOG_FILE, f"📥 ID={process_id} → зона сканирования: {actual_range}")
                    scan_process(row)
                    update_last_scan("SheetsInfo", process_id)
                else:
                    log_to_file(SHEETS_LOG_FILE, f"⚠️ ID={process_id} ошибка диапазона: {error}")
