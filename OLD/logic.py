...
from project_root.methods import PROCESSORS
...
############################################################################################################################################################
    ...
    def update_phase(self):
        log_to_file(log_file, "=" * 100)
        log_to_file(log_file, "=== 🔄 Старт фазы обновления ===")
        log_to_file(log_file, "=" * 100)
        log_to_file(log_file, "")
        ...
        if not tasks_to_update:
            log_to_file(log_file, "⚪ Нет задач для обновления.")
            log_to_file(log_file, "")
            log_to_file(log_file, "=" * 100)
            log_to_file(log_file, "")
            return

        # Разделение задач на три группы
        update_main_tasks = []
        update_shuffle_tasks = []
        update_other_tasks = []

        for task in tasks_to_update:
            if task.update_group == "update_main":
                update_main_tasks.append(task)
            elif task.update_group in ("update_generic_shuffle", "update_legendz_shuffle", "update_gsbj_shuffle"):
                update_shuffle_tasks.append(task)
            else:
                update_other_tasks.append(task)

        # Обрабатываем каждый тип задач отдельно
        if update_main_tasks:
            self._handle_main_updates(update_main_tasks)

        if update_shuffle_tasks:
            self._handle_shuffle_updates(update_shuffle_tasks)

        if update_other_tasks:
            self._handle_other_updates(update_other_tasks)

        log_to_file(log_file, "")
        log_to_file(log_file, "=" * 100)
        log_to_file(log_file, "✅ Фаза обновления завершена.")
        log_to_file(log_file, "=" * 100)
        log_to_file(log_file, "")

    def _handle_main_updates(self, tasks):
        log_to_file(log_file, "=" * 100)
    ...
    def _handle_shuffle_updates(self, tasks):
        log_to_file(log_file, "=" * 100)
    ...
    def _handle_other_updates(self, tasks):
        log_to_file(log_file, "=" * 100)
