...
from core.config import ROTATIONSINFO_LOG, ROTATIONSINFO_INTERVAL, ROTATION_ORDER
...
#############################################################################################
    ...
    def scan_phase(self):
        ...
        for task in self.tasks:
            log_to_file(self.log_file, f"⚪ [Task {task.name_of_process} {task.source_page_name}] Отсканировано: {task.scanned} | Обработано: {task.proceed} | Изменено: {task.changed} | Загружено: {task.uploaded}")
...
#############################################################################################
    ...
    def process_phase(self):
        ...
        for task in self.tasks:
            if task.scanned == 0:
                log_to_file(self.log_file, f"⚪ [Task {task.name_of_process} {task.source_page_name}] Задача не была успешно отсканирована. Пропуск.")
                continue
    ...
    def import_main_data(self, all_main_tasks):
        ...
        for page_name, tasks in grouped_by_page.items():
            ...
            for name in ROTATION_ORDER:
                ...
                if not task:
                    log_to_file(self.log_file, f"⚠️ Задача '{name}' {task.source_page_name}] не найдена.")
                    continue

                values = task.values_json
                if not values or not isinstance(values, list):
                    log_to_file(self.log_file, f"⚪ [Task {name} {task.source_page_name}] нет корректных данных. Пропуск.")
